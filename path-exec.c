 ...
 * @info: reference to the information structure
 * @pathway: path to assess
 *
 * Return: 1 on being executable, 0 if not
 */
int determine_if_executable(info_t *info, char *pathway)
{
	struct stat buf;
 ...
 *
 * Return: the full path if command is executable or NULL
 */
char *locate_command_path(info_t *info, char *path_env_var, char *command_name)
{
	int i = 0, start = 0;
	char *full_path;
...
if (_strlen(command_name) > 2 && starts_with(command_name, "./"))
	{
if (determine_if_executable(info, command_name))
	return (command_name);
	}
...
if (determine_if_executable(info, full_path))
	return (full_path);
